...
getCIs<-function(corrng, confidence = 0.95) {
  ...
  res2 <- new("CorMatrix", coord1 = data.frame(corrng[[1]],corrng[[2]],corrng[[3]]), coord2 = data.frame(corrng[[1]],corrng[[2]],corrng[[3]]), cormat = as.data.frame(cortbl[, , start_index]))
  res3 <- new("CorMatrix", coord1 = data.frame(corrng[[1]],corrng[[2]],corrng[[3]]), coord2 = data.frame(corrng[[1]],corrng[[2]],corrng[[3]]), cormat = as.data.frame(cortbl[, , end_index]))
  res4 <- new("CorMatrix", coord1 = data.frame(chr,start,end), coord2 = data.frame(chr,start,end), cormat = as.data.frame(cortbl[, , 1+replicates/2]))
